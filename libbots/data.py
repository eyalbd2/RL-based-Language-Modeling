...
def encode_phrase_pairs(phrase_pairs, emb_dict, filter_unknows=True):
    ...
    result = []
    for q, p in phrase_pairs:
        pq = encode_words(q, emb_dict), encode_words(p, emb_dict)
        if unk_token in pq[0] or unk_token in pq[1]:
            continue
        result.append(pq)
    return result
...
def yield_samples(data):
    assert isinstance(data, list)

    ofs = 0
    while True:
        batch = data[ofs:(ofs+1)]
        if len(batch) <= 1:
            break
        yield batch
        ofs += 1
...
def decode_words(indices, rev_emb_dict):
    return [rev_emb_dict.get(int(idx), UNKNOWN_TOKEN) for idx in indices]
